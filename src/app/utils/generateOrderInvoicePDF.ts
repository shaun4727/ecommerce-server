...
export const generateOrderInvoicePDF = async (order: IOrder): Promise<Buffer> => {
	return new Promise<Buffer>(async (resolve, reject) => {
		try {
			...
			doc.text(
				`Shipping Address: ${order.shippingAddress.area}, ${order.shippingAddress.city} ${order.shippingAddress.zip_code}, Bangladesh`,
			);
			doc.moveDown(1);
			...
			doc.fontSize(11)
				.font('Helvetica-Bold')
				.fillColor('#003366')
				.text(`${order.finalAmount.toFixed(2)} /-`, 400, pricingY, { width: 90, align: 'right' });
			pricingY += tableHeight;
