...
const createProduct = async (productData: Partial<IProduct>, productImages: IImageFiles, authUser: IJwtPayload) => {
...
const getAllProduct = async (query: Record<string, unknown>) => {
	const { minPrice, maxPrice, categories, brands, inStock, ratings, ...pQuery } = query;
	...
	// Filter by brands
	if (brands) {
		const brandArray = typeof brands === 'string' ? brands.split(',') : Array.isArray(brands) ? brands : [brands];
	...
	if (ratings) {
		const ratingArray =
			typeof ratings === 'string' ? ratings.split(',') : Array.isArray(ratings) ? ratings : [ratings];

		// filter.averageRating = { $in: ratingArray.map(Number) };
		const ratingConditions = ratingArray
			.map((ratingStr) => {
	...
	const productQuery = new QueryBuilder(
		Product.find(filter).populate('category', 'name').populate('shop', 'shopName').populate('brand', 'name'),
		pQuery,
	)
		.search(['name', 'description'])
		.filter()
