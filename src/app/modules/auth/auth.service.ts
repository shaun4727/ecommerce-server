...
const forgotPassword = async ({ email }: { email: string }) => {
	...
	try {
		...
		throw new AppError(StatusCodes.INTERNAL_SERVER_ERROR, 'Failed to send OTP email. Please try again later.');
	}
};
...
const verifyOTP = async ({ email, otp }: { email: string; otp: string }) => {
	...
	if (!user.otpToken || user.otpToken === '') {
		throw new AppError(StatusCodes.BAD_REQUEST, 'No OTP token found. Please request a new password reset OTP.');
	}
...
const resetPassword = async ({ token, newPassword }: { token: string; newPassword: string }) => {
	...
	try {
		...
		const hashedPassword = await bcrypt.hash(String(newPassword), Number(config.bcrypt_salt_rounds));
