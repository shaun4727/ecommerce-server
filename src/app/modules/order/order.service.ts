...
const getMyOrders = async (query: Record<string, unknown>, authUser: IJwtPayload) => {
	const orderQuery = new QueryBuilder(
		Order.find({ user: authUser.userId }).populate([
			...
			{
				path: 'assigned',
				model: 'AgentOrder', // Explicitly tell Mongoose to look in AgentOrder
				populate: {
					path: 'agentId', // Populate the User details inside the AgentOrder
					model: 'User',
					select: 'name email role', // Optional: only get specific fields
				},
			},
		]),
		query,
